/*
	node-ps-client.js - Node JS Pokemon Showdown Client library
	
	By: Ecuacion (https://github.com/Ecuacion) (C) Copyright 2015
...
function Client (server, port, opts) {
	...
	self.actionUrl = url.parse(self.opts.loginServer);
	...
	for (var i in Client.prototype) self[i] = Client.prototype[i];
	
	if (self.opts.autoConnect) {
		self.connect();
	}
	
	if (self.opts.connectionTimeout) {
...
Client.prototype.rename = function (nick, pass) {
	...
	if (!pass) {
		...
		var data = 'act=login&name=' + nick + '&pass=' + pass + '&challengekeyid=' + this.challstr.id + '&challenge=' + this.challstr.str;
...
Client.prototype.receiveLine = function (room, message, isIntro) {
	...
	switch (spl[0]) {
		...
		case 'J': case 'j':
			var by = spl[1];
			if (this.rooms[room]) this.rooms[room].users[toId(by)] = by;
		...
		case 'l': case 'L':
			var by = spl[1];
			if (this.rooms[room] && this.rooms[room].users[toId(by)]) delete this.rooms[room].users[toId(by)];
